...
inline float scan1Inclusive(float idata, __local float *l_Data, uint size)
{
...
inline float scan1Exclusive(float idata, __local float *l_Data, uint size)
{
...
inline float warpScanInclusive(float idata, __local float *l_Data, uint size)
{
	uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
	l_Data[pos] = 0;
	pos += size;
...
inline float warpScanExclusive(float idata, __local float *l_Data, uint size)
{
...
inline float scan1Inclusive(float idata, __local float *l_Data, uint size)
{
	if(size > WARP_SIZE)
	{
		...
		float warpResult = warpScanInclusive(idata, l_Data, WARP_SIZE);
		...
		if( get_local_id(0) < (WORKGROUP_SIZE / WARP_SIZE) )
		{
			...
			float val = l_Data[get_local_id(0)];
...
//in work-item private memory as float4
inline float4 scan4Inclusive(float4 data4, __local float *l_Data, uint size)
{
	...
	float val = scan1Inclusive(data4.w, l_Data, size / 4) - data4.w;

	return (data4 + (float4)val);
}

inline float4 scan4Exclusive(float4 data4, __local float *l_Data, uint size)
{
...
void scanExclusiveLocal1(__global float4 *d_Dst,
                         __global float4 *d_Src,
						 __local float *l_Data,
						 uint size)
{
	//Load data
	float4 idata4 = d_Src[get_global_id(0)];

	//Calculate exclusive scan
	float4 odata4  = scan4Exclusive(idata4, l_Data, size);
...
__kernel __attribute__((reqd_work_group_size(WORKGROUP_SIZE, 1, 1)))
void scanExclusiveLocal2(__global float *d_Buf,
						 ...
						 uint arrayLength)
{
	...
	//Compute
	float odata = scan1Exclusive(data, l_Data, arrayLength);
...
__kernel __attribute__((reqd_work_group_size(WORKGROUP_SIZE, 1, 1)))
void uniformUpdate(__global float4 *d_Data,
                   __global float *d_Buf)
{
	__local float buf[1];

	float4 data4 = d_Data[get_global_id(0)];
